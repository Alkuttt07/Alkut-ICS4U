...
class Item:
    def __init__(self, name, description, item_type, value, image, ID, dimensions = (1,1)): #size refers to the literal shape/area the item takes in inv 
        self.name = name
...
class Potion(Item):
    def __init__(self, name, description, value, image, ID, dimensions = (1,1)):
        super().__init__(name, description, "potion", value, image, ID, dimensions)


class Weapon(Item):
    def __init__(self, name, description, value, image, ID, damage, cooldown, hitchance, dimensions = (1,1)):
        super().__init__(name, description, "weapon", value, image, ID, dimensions)
        self.damage = damage
...
#examples
w1 = Weapon("blablabla","good weapon",50,None,35,20,10,0.8,(2,1))
w2 = Weapon("sss","better waepon",100,None,50,30,10,0.9,(1,3))
w3 = Weapon("bbb0","best weapon",150,None,60,40,13,1,(2,2))
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            if event.button == 1:
                if pygame.Rect(
                    ...
                    continue
                if pygame.Rect(
                    upgrade_button.pos[0],
                    upgrade_button.pos[1],
                    upgrade_button.size[0],
                    upgrade_button.size[1]
                ).collidepoint(mouse_x, mouse_y):
                    continue
